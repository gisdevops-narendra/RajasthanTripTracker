...
function doGet(e) {
  try {
    ...
    switch (action) {
      ...
      case "bulkSaveAll":
        result = bulkSaveAll(payload.days);
        break;

      case "bulkSavePlaces":
        result = bulkSavePlaces(payload.days);
        break;
...
// Bulk writers used for the initial cloud upload and for "Reset Trip":
// one setValues() per sheet instead of one appendRow()/getRange() round trip
// per day/place/bill, which is what made those two flows slow.
function bulkSaveAll(days) {
  if (!Array.isArray(days)) throw new Error("Invalid days");
  const sheets = setupSheets();
  ...
  days.forEach(day => {
    dayRows.push([day.id, day.name || "Day"]);

    (day.places || []).forEach(place => {
      const row = [
        day.id, day.name || "Day", place.id, place.sourceKey || "",
        place.original === true, place.name || "Sightseeing",
        place.visited === true, place.paidBy || "Not Paid"
      ];
      PEOPLE_IDS.forEach(id => row.push(Number(place.costs && place.costs[id]) || 0));
      placeRows.push(row);
    });

    (day.foodBills || []).forEach(bill => {
      billRows.push([
        day.id, day.name || "Day", bill.id, bill.date || "",
        bill.restaurantShop || "", bill.food || "", bill.category || "lunch",
        Number(bill.amount) || 0, bill.paidBy || "parul",
        bill.paymentStatus || "paid", bill.notes || ""
      ]);
    });
  });

  writeSheetBulk(sheets.days, dayRows);
  writeSheetBulk(sheets.places, placeRows);
  writeFoodBillsBulk(sheets.foodBills, billRows);

  return {message: "Bulk save complete"};
}

function bulkSavePlaces(days) {
  if (!Array.isArray(days)) throw new Error("Invalid days");
  const sheets = setupSheets();
  ...
  writeSheetBulk(sheets.places, placeRows);
  return {message: "Places bulk saved"};
}

function writeSheetBulk(sheet, rows) {
  clearDataSheet(sheet, 2);
  if (!rows.length) return;
  sheet.getRange(2, 1, rows.length, rows[0].length).setValues(rows);
}

function writeFoodBillsBulk(sheet, rows) {
  clearDataSheet(sheet, 2);
  if (!rows.length) return;
  ...
  dateRange.setNumberFormat("@").setValues(rows.map(r => [r[3]]));
}
